    ...
    try {
        ...
        if (householdId) {
            const existingOrderQuery = `
                ...
                WHERE household_id = $1 AND delivery_date = $2 AND status = 'Pending'
            `;
        ...
        // Insert or update items in the order
        for (const item of processedItems) {
            const existingItemQuery = `
                SELECT * FROM order_items
                WHERE order_id = $1 AND product_id = $2 AND user_id = $3
            `;
            const existingItemResult = await client.query(existingItemQuery, [orderId, item.product_id, userId]);
            ...
            if (existingItemResult.rows.length > 0) {
                ...
                await client.query(
                    `UPDATE order_items
                     ...
                     WHERE order_id = $3 AND product_id = $4 AND user_id = $5`,
                    [newQuantity, newSubtotal, orderId, item.product_id, userId]
                );
            } else {
                await client.query(
                    `INSERT INTO order_items (order_id, product_id, user_id, quantity, unit_price, subtotal)
                     VALUES ($1, $2, $3, $4, $5, $6)`,
                    [orderId, item.product_id, userId, item.quantity, item.unit_price, item.subtotal]
                );
            }
        }

        // Fetch all related orders for the household on the same date
        ...
        await client.query("COMMIT");
        console.log(`Order successfully processed for ID: ${orderId}`);
